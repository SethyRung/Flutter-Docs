...
- Sharing state between widgets using constructors, `InheritedWidget`s, and callbacks
...
- Video: [Managing state in Flutter](https://youtube.com/watch?v=vU9xDLdEZtU). This video shows how to use the [riverpod](https://pub.dev/packages/riverpod) package.
- Tutorial: [State management](https://docs.flutter.dev/data-and-backend/state-mgmt/intro). This shows how to use `ChangeNotifer` with the [provider](https://pub.dev/packages/provider) package.
...
The `_MyCounterState` object and its `count` variable are created the first time that `MyCounter` is built, and exist until it's removed from the screen. This is an example of *ephemeral state*.

You might find the following resources to be useful:

- Article: [Ephemeral state and app state](https://docs.flutter.dev/data-and-backend/state-mgmt/ephemeral-vs-app)
...
- **Using** `InheritedWidget` (or a similar API, such as the [provider](https://pub.dev/packages/provider) package).
...
Passing the shared data for your app through widget constructors makes it clear to anyone reading the code that there are shared dependencies. Creating a chain of widgets that only pass data down to their children is often referred to as&#x20;[]{.tooltip-wrapper style="box-sizing: border-box; border-width: 0px; position: relative;"&#x7D;&#x5B;*[prop drilling](https://docs.flutter.dev/resources/glossary#prop-drilling)*]{.tooltip-target style="box-sizing: border-box; border-width: 0px;"}&#x5B;]{.tooltip style="box-sizing: border-box; border: 0.05rem solid rgba(0, 0, 0, 0.125); visibility: hidden; display: block; position: absolute; z-index: 10; top: 44.4px; left: 0px; transform: translateX(-50%); width: 16rem; background: none 0% 0% / auto repeat scroll padding-box border-box rgb(28, 30, 39); border-radius: 0.75rem; box-shadow: rgba(0, 0, 0, 0.15) 0px 0.5rem 1rem; padding: 0.8rem; font-size: 0.875rem; font-weight: normal; font-style: normal;"}[]{.tooltip-content style="box-sizing: border-box; border-width: 0px; display: flex; flex-flow: column; color: rgb(220, 220, 220);"}[]{.tooltip-header style="box-sizing: border-box; border-width: 0px; font-size: 1.2rem; font-weight: 500; margin-bottom: 0.25rem;"}[]{style="box-sizing: border-box; border-width: 0px;"}.

### Using InheritedWidget

Manually passing data down the widget tree can be verbose and cause unwanted boilerplate code, so Flutter provides `InheritedWidget`, which provides a way to efficiently host data in a parent widget so that child widgets can access them without storing them as a field.
...
- Video: [Pragmatic state management](https://youtube.com/watch?v=d_m5csmrf7I)
- Video: [InheritedWidgets](https://youtube.com/watch?v=og-vJqLzg2c)
- Video: [A guide to Inherited Widgets](https://youtube.com/watch?v=Zbm3hjPjQMk)
- Sample: [Provider shopper](https://github.com/flutter/samples/tree/main/provider_shopper)
...
As this section of the website is evolving, we [welcome your feedback](https://google.qualtrics.com/jfe/form/SV_6A9KxXR7XmMrNsy?page=%22state-management%22)!
