...
description: Set up Flutter on your device with a OSS-based editor, such as VS Code, and get started developing your first multi-platform app with Flutter!
navigation:
...
::card{icon="i-lucide-info" title="What you'll achieve"}
...
6. Click **Clone Flutter**. :br While downloading Flutter, VS Code displays this pop-up notification: :br`Downloading the Flutter SDK. This may take a few minutes.`:br This download takes a few minutes. If you suspect that the download has hung, click Cancel then start the installation again.
...
1. Open the command palette in VS Code. :br Go to **View** > **Command Palette** or press `Control` + `Shift` + `P`.
2. In the command palette, start typing `flutter:`. :br VS Code should surface commands from the Flutter plugin.
3. Select the **Flutter: New Project** command. :br Your OS or VS Code might ask for access to your documents, agree to continue to the next step.
4. Choose the Application template. :br VS Code should prompt you with **Which Flutter template?**. Choose **Application** to bootstrap a simple counter app.
5. Create or select the parent directory for your new app's folder. :br A file dialog should appear.
...
6. Enter a name for your app. :br VS Code should prompt you to enter a name for your new app. Enter `trying_flutter` or a similar `lowercase_with_underscores` name. To confirm your selection, press `Enter`.
7. Wait for project initialization to complete. :br Task progress is often surfaced as a notification in the bottom right and can also be accessed from the **Output** panel.
8. Open the `lib` directory, then the `main.dart` file. :br If you're curious about what each portion of the code does, check out the preceding comments throughout the file.
...
1. In the running app, try adding to the counter a few times by clicking the increment :u-button{icon="i-lucide-plus" size="xs"} button.
2. With your app still running, make a change in the `lib/main.dart` file. :br Change the `_counter++` line in the `_incrementCounter` method to instead decrement the `_counter`field.
   ```dart
   setState(() {
     // ...
     _counter++;
   ...
   ```
3. Save your changes (**File** > **Save All**) or click the **Hot Reload** :u-icon{icon="i-lucide-zap"} button. :br Flutter updates the running app without losing any existing state. Notice the existing value stayed the same.
4. Try clicking the increment :u-button{icon="i-lucide-plus" size="xs"} button again. Notice the value decreases instead of increases.
...
1. If your app isn't running, start debugging it again. :br Go to **Run** > **Start Debugging** or press `F5`.
2. Open the Flutter sidebar in VS Code. :br Either open it with the Flutter ![flutter icon](https://docs.flutter.dev/assets/images/branding/flutter/logo/square.svg){.size-6.inline-block} button in the VS Code sidebar or open it from the command palette by running the **Flutter: Focus on Flutter Sidebar View** command.
3. In the Flutter sidebar, under **DevTools**, click the **Flutter Inspector** button. :br A separate **Widget Inspector** panel should open in VS Code. :br In the widget inspector, you can view your app's widget tree, view the properties and layout of each widget, and more.
4. In the widget inspector, try clicking the top-level `MyHomePage` widget. :br A view of its properties and layout should open, and the VS Code editor should navigate to and focus the line where the widget was included.
